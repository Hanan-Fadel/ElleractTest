  ...
  render() {
    return (
      <InputGroup className="mb-3">
        <Input id="searchField" placeholder="Search appointments by nurse name" type="text" className="form-control" onChange={this.handleSearch} />
        <InputGroupButtonDropdown addonType="append" isOpen={this.state.dropdownOpen} toggle={this.toggleDropDown}>
          <DropdownToggle caret color="primary">
            Sort by
            </DropdownToggle>
          <DropdownMenu right>
            <DropdownItem onClick={this.handleSort} id="nurseName">Nurse Name {(this.props.orderBy === 'nurseName') ? <span className="fas fa-check"></span> : null}</DropdownItem>
            <DropdownItem onClick={this.handleSort} id="aptDate">Date {(this.props.orderBy === 'aptDate') ? <span className="fas fa-check"></span> : null}</DropdownItem>
